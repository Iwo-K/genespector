...
def import_adata(adata, use_raw = True, make_var_unique = True):
    ...
    if make_var_unique: adata.var_names_make_unique()
    ...
    if use_raw:
        genekeys = adata.raw.var.index.values #Names of the columns in the gene expression array
    else:
        genekeys = adata.var.index.values

    #Extracting numerical data from the .obs slot
    numcols = [str(x) in ['float64', 'float32', 'in32', 'int64'] for x in adata.obs.dtypes]
    ...
    for i in adata.uns.keys():
        ...
        if check == 'colors':
            ...
            color_catinfo[category] = {x : adata.uns[i][n] for n,x in enumerate(adata.obs[category].cat.categories)}
